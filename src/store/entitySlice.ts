	...
	reducers: {
		...
		updateLecturer(state, action: PayloadAction<{ id: string; updates: UpdateLecturerInput }>) {
			lecturersAdapter.updateOne(state.lecturers, {
				...
				changes: { ...action.payload.updates, updatedAt: new Date() },
			})
		},
...
export const updateLecturerThunk = createAsyncThunk<
	void,
	{ id: string; updates: UpdateLecturerInput },
	{ state: RootState }
>('entities/updateLecturerThunk', async ({ id, updates }, { dispatch, getState }) => {
	if (selectIsDemoMode(getState())) {
		dispatch(updateLecturer({ id, updates }))
		return
	}
	const updated = await lecturersApi.update(id, updates)
	dispatch(upsertLecturer(updated))
})
...
export const updateRoomThunk = createAsyncThunk<
	void,
	{ id: string; updates: UpdateRoomInput },
	{ state: RootState }
>('entities/updateRoomThunk', async ({ id, updates }, { dispatch, getState }) => {
	if (selectIsDemoMode(getState())) {
		dispatch(updateRoom({ id, updates }))
		return
	}
	const updated = await roomsApi.update(id, updates)
	dispatch(upsertRoom(updated))
})
...
export const updateFacultyThunk = createAsyncThunk<
	void,
	{ id: string; updates: UpdateFacultyInput },
	{ state: RootState }
>('entities/updateFacultyThunk', async ({ id, updates }, { dispatch, getState }) => {
	if (selectIsDemoMode(getState())) {
		dispatch(updateFaculty({ id, updates }))
		return
	}
	const updated = await facultiesApi.update(id, updates)
	dispatch(upsertFaculty(updated))
})
