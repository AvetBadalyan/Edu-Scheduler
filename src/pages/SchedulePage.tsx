...
import { selectHasSchedule, loadSchedule, resetSchedule, moveClass } from '@/store/scheduleSlice'
import { selectVizPlaybackState, selectVizResult, resetVisualization } from '@/store/visualizationSlice'
import type { ScheduleInput, ScheduleResult, TimeSlotRef } from '@/types'
import { BrainCircuit, CalendarCheck, CheckCircle2, Eye, GripVertical, Redo2, RotateCcw, Sparkles, Undo2, Zap } from 'lucide-react'
...
function StatCard({ label, value, icon: Icon, gradient }: { label: string; value: number; icon: typeof CalendarCheck; gradient: string }) {
  return (
    <div className={cn('relative overflow-hidden rounded-2xl bg-gradient-to-br p-4 text-white shadow-md', gradient)}>
...
export default function SchedulePage() {
  ...
  const isVizActive = showVisualizer && (vizState === 'playing' || vizState === 'paused' || vizState === 'complete')

  const scheduleInput: ScheduleInput = useMemo(() => ({ lecturers, rooms, faculties }), [lecturers, rooms, faculties])

  const totalRequired = useMemo(
    () => faculties.reduce((sum, f) => sum + f.syllabus.reduce((s, e) => s + e.requiredHours, 0), 0),
    [faculties]
  )

  const handleGenerate = () => {
    if (showVisualizer) { setShowVisualizer(false); dispatch(resetVisualization()) }
  ...
  const handleSlotDrop = (from: TimeSlotRef, to: TimeSlotRef) => {
    ...
    if (!moveResult.success) toast.error(moveResult.error ?? 'Could not move class.')
  }

  return (
    <div className="flex flex-col gap-6">
      <PageHeader title="Schedule" description="Generate a conflict-free timetable, then drag classes to fine-tune."
        actions={
          <div className="flex flex-wrap gap-2">
            {hasSchedule && !showVisualizer && (
              <>
                <Button variant="outline" size="sm" onClick={undo} disabled={!canUndo} aria-label="Undo"><Undo2 className="size-3.5" /></Button>
                <Button variant="outline" size="sm" onClick={redo} disabled={!canRedo} aria-label="Redo"><Redo2 className="size-3.5" /></Button>
                <Button variant="outline" size="sm" onClick={handleReset}><RotateCcw className="size-3.5" /> Reset</Button>
              </>
            )}
            <Button variant="outline" onClick={handleToggleVisualizer} disabled={!canGenerate} aria-pressed={showVisualizer}>
              <Eye className="size-4" />{showVisualizer ? 'Hide visualizer' : 'Visualize'}
            </Button>
            <Button onClick={handleGenerate} disabled={!canGenerate || isGenerating}>
              <Sparkles className="size-4" />{isGenerating ? 'Generating…' : 'Generate schedule'}
            </Button>
          </div>
        }
      />
      ...
      {!canGenerate && (
        <div className="flex items-start gap-3 rounded-2xl border border-amber-200 bg-amber-50 px-5 py-4">
          ...
          <div>
            ...
            <p className="text-xs text-amber-700 mt-0.5">Add at least one lecturer, one room, and one faculty.</p>
          </div>
        </div>
      )}
      ...
      {result && !showVisualizer && (
        <div className="grid grid-cols-2 gap-3 sm:grid-cols-4 animate-fade-up">
          <StatCard label="Classes placed" value={countAssignments(result)} icon={CalendarCheck} gradient="from-indigo-600 to-violet-700" />
          <StatCard label="Backtracks"     value={result.backtracks}        icon={RotateCcw}    gradient="from-sky-600 to-blue-700" />
          <StatCard label="Algorithm steps" value={result.totalSteps}       icon={BrainCircuit} gradient="from-emerald-600 to-teal-700" />
          <StatCard label="Unresolved"     value={result.unresolvedConstraints.length} icon={Zap} gradient={result.unresolvedConstraints.length > 0 ? 'from-amber-600 to-orange-700' : 'from-emerald-600 to-teal-700'} />
        </div>
      )}
          ...
          <div className="flex flex-col gap-4">
            ...
            {vizState === 'complete' && vizResult && (
              <Button onClick={handleKeepResult} className="w-full gap-2">
                <CheckCircle2 className="size-4" />Use this schedule
              </Button>
            )}
          </div>
            ...
            {isVizActive ? (
              <>
                <div className="flex items-center gap-2 rounded-xl bg-indigo-50 border border-indigo-100 px-4 py-2.5 text-xs font-medium text-indigo-700">
                  <span className={cn('size-2 rounded-full', vizState === 'complete' ? 'bg-emerald-500' : 'bg-indigo-500 animate-pulse')} />
                  {vizState === 'complete' ? 'Algorithm finished — click "Use this schedule" to apply.' : 'Watching algorithm assign classes…'}
                </div>
              ...
              <div className="flex min-h-[20rem] flex-col items-center justify-center gap-4 rounded-2xl border border-dashed border-indigo-200 bg-indigo-50/30 text-center px-8">
                ...
                <div>
                  ...
                  <p className="mt-1 text-sm text-gray-500 max-w-xs">The timetable starts empty. Cells appear as the algorithm assigns classes — and disappear when it backtracks.</p>
                </div>
                <p className="rounded-full bg-indigo-100 px-4 py-1.5 text-xs font-semibold text-indigo-700">Click "Start Visualization" →</p>
              </div>
            )}
          </div>
        </div>
            ...
            <div className="overflow-hidden rounded-2xl border border-gray-100 bg-white shadow-sm animate-fade-up">
              <div className="relative overflow-hidden bg-gradient-to-br from-indigo-600 via-violet-600 to-indigo-700 px-8 py-12 text-center">
                ...
                <div className="relative">
                  ...
                  <p className="mt-2 text-sm text-white/80 max-w-sm mx-auto">The constraint-satisfaction solver will place every subject into a conflict-free slot in seconds.</p>
                  {canGenerate && (
                    <div className="mt-6 flex flex-col items-center gap-3 sm:flex-row sm:justify-center">
                      <button onClick={handleGenerate} className="flex items-center gap-2 rounded-xl bg-white px-6 py-2.5 text-sm font-bold text-indigo-700 shadow-lg transition-all hover:shadow-xl hover:-translate-y-0.5">
                        <Sparkles className="size-4" />Generate instantly
                      </button>
                      <button onClick={handleToggleVisualizer} className="flex items-center gap-2 rounded-xl border border-white/30 bg-white/10 px-6 py-2.5 text-sm font-semibold text-white backdrop-blur-sm transition-all hover:bg-white/20">
                        <Eye className="size-4" />Watch it build
                      </button>
                    </div>
                  )}
                </div>
              ...
              <div className="grid divide-x divide-gray-100 sm:grid-cols-3">
                {[
                  { icon: BrainCircuit, title: 'Smart algorithm', desc: 'Backtracking solver handles complex constraints automatically.' },
                  { icon: Eye, title: 'Step-by-step viz', desc: 'Watch every assignment and backtrack in real time.' },
                  { icon: GripVertical, title: 'Drag to edit', desc: 'Fine-tune any slot with drag-and-drop after generation.' },
