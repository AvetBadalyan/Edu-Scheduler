...
export function Toaster() {
  ...
  return (
    <div className="fixed bottom-4 right-4 z-50 flex flex-col gap-2 max-w-sm w-full"
      ...
      {toasts.map(toast => (
        <div key={toast.id}
          className={cn('flex items-start gap-3 rounded-lg border px-4 py-3 shadow-lg', VARIANT_STYLES[toast.variant])}
          role="alert">
          <span className="text-sm font-medium" aria-hidden>{VARIANT_ICONS[toast.variant]}</span>
