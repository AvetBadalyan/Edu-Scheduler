...
export function FacultyForm({
	...
	const validate = () => {
		...
		if (valid.some(s => s.requiredHours < 1)) e.syllabus = 'Each subject needs at least 1 hour.'
		setErrors(e)
		...
		<form onSubmit={handleSubmit} className="flex flex-col gap-5" noValidate>
			...
			<div className="flex items-center gap-3">
				...
				<div>
					...
					<p className="text-xs text-gray-500">Configure syllabus and students</p>
				</div>
			</div>
					...
					{syllabus.map((entry, i) => (
						<div key={entry._key} className="grid grid-cols-[1fr_5rem_2rem] gap-2 items-center">
							...
							<Input
								...
								onChange={e => updateEntry(i, 'requiredHours', parseInt(e.target.value, 10) || 1)}
			...
			<div className="flex justify-end gap-2">
				...
				<Button type="submit" disabled={isLoading}>
					{isLoading ? 'Saving…' : mode === 'create' ? 'Add Faculty' : 'Save Changes'}
				</Button>
			</div>
		</form>
	)
