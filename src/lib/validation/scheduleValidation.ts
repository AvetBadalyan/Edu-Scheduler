...
export function validateLecturer(l: Lecturer, index: number): ValidationError[] {
  ...
  if (!l.name?.trim()) {
    errors.push(error("LECTURER_MISSING_NAME", `Lecturer at index ${index} has no name.`, `${field}.name`))
  }
  if (!l.surname?.trim()) {
    errors.push(error("LECTURER_MISSING_SURNAME", `Lecturer at index ${index} has no surname.`, `${field}.surname`))
  }
  if (!l.specialties || l.specialties.length === 0) {
    errors.push(error("LECTURER_NO_SPECIALTIES", `Lecturer "${l.name ?? index}" has no specialties.`, `${field}.specialties`))
  }
  if (l.specialties?.some((s) => !s?.trim())) {
    errors.push(error("LECTURER_EMPTY_SPECIALTY", `Lecturer "${l.name ?? index}" has an empty specialty entry.`, `${field}.specialties`))
  }
...
export function validateRoom(r: Room, index: number): ValidationError[] {
  ...
  if (!r.number?.trim()) {
    errors.push(error("ROOM_MISSING_NUMBER", `Room at index ${index} has no room number.`, `${field}.number`))
  }
  if (!Number.isInteger(r.capacity) || r.capacity < 1 || r.capacity > 500) {
    errors.push(error("ROOM_INVALID_CAPACITY", `Room "${r.number ?? index}" must have a capacity between 1 and 500.`, `${field}.capacity`))
  }
...
export function validateFaculty(f: Faculty, index: number): ValidationError[] {
  ...
  if (!f.name?.trim()) {
    errors.push(error("FACULTY_MISSING_NAME", `Faculty at index ${index} has no name.`, `${field}.name`))
  }
  if (!f.syllabus || f.syllabus.length === 0) {
    errors.push(error("FACULTY_EMPTY_SYLLABUS", `Faculty "${f.name ?? index}" has no syllabus entries.`, `${field}.syllabus`))
  }
  f.syllabus?.forEach((entry, ei) => {
    if (!entry.subject?.trim()) {
      errors.push(error("SYLLABUS_MISSING_SUBJECT", `Faculty "${f.name ?? index}" syllabus entry ${ei} has no subject.`, `${field}.syllabus[${ei}].subject`))
    }
    if (!Number.isInteger(entry.requiredHours) || entry.requiredHours < 1) {
      errors.push(error("SYLLABUS_INVALID_HOURS", `Faculty "${f.name ?? index}" subject "${entry.subject}" must have at least 1 required hour.`, `${field}.syllabus[${ei}].requiredHours`))
    }
  })
  if (!f.students || f.students.length === 0) {
    errors.push(error("FACULTY_NO_STUDENTS", `Faculty "${f.name ?? index}" has no students.`, `${field}.students`))
  }
...
export function validateSpecialtyMatches(
  ...
  const allSubjects = new Set(
    faculties.flatMap((f) => f.syllabus.map((s) => s.subject))
  )
