...
universitiesRouter.get('/:id', async (req: AuthRequest, res, next) => {
	try {
		const university = await AppDataSource.getRepository(University).findOneBy({
			id: req.params.id,
		})
		if (!university)
			return res.status(404).json({ code: 'NOT_FOUND', message: 'University not found.' })
...
universitiesRouter.post('/', async (req: AuthRequest, res, next) => {
	try {
		...
		if (!name?.trim())
			return res
				.status(400)
				.json({ code: 'VALIDATION_ERROR', message: 'University name is required.' })
